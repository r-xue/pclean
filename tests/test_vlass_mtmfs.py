...
VLASS_QL_MS = ctsys.resolve(
    'pl-regressiontest/vlass_quicklook/TSKY0001.sb32295801.eb32296475.57549.31722762731_split_withcorrectdata.ms'
)
...
def test_vlass_mtmfs_mosaic_parallel(tmp_path, monkeypatch):
    ...
    for tt in range(2):
        assert os.path.isdir(f'{imagename}.psf.tt{tt}'), f'PSF tt{tt} not found'
        assert os.path.isdir(f'{imagename}.residual.tt{tt}'), f'Residual tt{tt} not found'
